...
class Telegram:
            ...
            try:
                async with session.get(
                    ...
                    if jar.filter_cookies(embed_response.url).get('stel_ssid'):
                        views_token = finditer('data-view="([^"]+)"', await embed_response.text())
                        views_token = next(views_token, None)
                        if views_token:
    ...
    def run_proxies_tasks(self, lines: list, proxy_type):
        async def inner(proxies: list):
            await asyncio.gather(
                *[self.request(proxy, proxy_type) for proxy in proxies]
            )
    ...
    async def run_rotated_task(self, proxy, proxy_type):
        while True:
            await asyncio.gather(
                *[self.request(proxy, proxy_type) for _ in range(self.tasks)]
            )
    ...
    def cli(self):
        ...
        Sent: {self.success_sent}
        Fail: {self.failed_sent}
...
class Auto:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    for proxy in finditer(REGEX, html):
    ...
    async def init(self):
        ...
        for sources in (
            ...
            tasks.extend([
                self.scrap(source_url, proxy_type) for source_url in srcs
            ])
...
def main():
    ...
    parser.add_argument('-p', '--proxy', dest='proxy', help='Proxy file path or user:password@host:port', type=str,
                        required=False)
